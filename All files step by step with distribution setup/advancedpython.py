...
class Girl(Character):
    ...
    def deterministic_move(self, minx, maxx):
        if self.rect.x <= minx or self.rect >= maxx:
            self.velocity *= -1
    ...
    def proximity_check(self, zombiex, zombiey):
        if zombiey == self.rect.y:
            if abs(zombiex - self.rect.x) < 250:
                return True
        return False
...
class Level(object):
    ...
    def get_tile_rects(self):
        tile_rects =[]
        for tile in self.objects_tiles[self.id]:
...
def main():
    ...
    while True:
        ...
        if girl.proximity_check(zombie.rect.x, zombie.rect.y):
            ...
            #girl.deterministic_move(x, y)   ##Deterministic AI
            girl.amble(x, y)    ##Non-Deterministic AI
